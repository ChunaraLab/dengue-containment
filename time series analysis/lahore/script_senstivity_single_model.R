...
### code ###
town.1<-read.csv(paste("data/",direct,"/Aziz Bhati Town.csv",sep=""))
town.2<-read.csv(paste("data/",direct,"/Cantonment.csv",sep=""))
town.3<-read.csv(paste("data/",direct,"/Data Ganj Bakhsh Town.csv",sep=""))
town.4<-read.csv(paste("data/",direct,"/Gulberg Town.csv",sep=""))
town.5<-read.csv(paste("data/",direct,"/Allama Iqbal Town.csv",sep=""))
town.6<-read.csv(paste("data/",direct,"/Nishter Town.csv",sep=""))
town.7<-read.csv(paste("data/",direct,"/Ravi Town.csv",sep=""))
town.8<-read.csv(paste("data/",direct,"/Samanabad Town.csv",sep=""))
town.9<-read.csv(paste("data/",direct,"/Shalamar Town.csv",sep=""))
town.10<-read.csv(paste("data/",direct,"/Wahga Town.csv",sep=""))
town.pop<-read.csv("data/lahore_population_area_towns.csv")
weather.lahore<-read.csv("data/weather_Lahore.csv")


town.pop<-adjust.pop(town.pop)

weather<-data.frame(date=weather.lahore$date, max_temperature=weather.lahore$max_temperature, min_temperature=weather.lahore$min_temperature, min_rain_fall=weather.lahore$min_rain_fall)
...
while (repl <=100){
  ...
  fin.data<-rbind(t1,t2,t3,t4,t5,t6,t7,t8,t9,t10)
...
#save(fin.data,file='lhr_single_replication_models_dfs_all_years_norm_area.Rdata')
...
model_seperate_x<-scam(y~x1+x2+x3+x4+x5+x6+x7+x8+x9+x10+s(lag.irs,bs="mdcx")+s(lag.Larviciding,bs="mdcx")+s(lag.Fogging,bs="mdcx")+s(lag.Tyres,bs="mdcx")+s(lag.max_temperature,bs="micv")+s(lag.min_rain_fall,bs="micv")+s(lag.Dewatering,bs="mdcx")+s(lag.FishSeed,bs="mdcx")+s(lag.Taps,bs="mdcx") + s(pop.density,bs="mpi") -1,data=pred_df)

model_seperate_x_no_contain<-scam(y~x1+x2+x3+x4+x5+x6+x7+x8+x9+x10+s(lag.max_temperature,bs="micv")+s(lag.min_rain_fall,bs="micv")+ s(pop.density,bs="mpi") -1,data=pred_df)
